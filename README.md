...
For example, to use the latest stable version (`v3.5.0`) you can execute the following command:

```
$ kubectl apply -f https://raw.githubusercontent.com/cloudscale-ch/csi-cloudscale/master/deploy/kubernetes/releases/csi-cloudscale-v3.5.0.yaml
```
...
master with the version `v3.5.0` and then publish a new docker build:
...
This will create a binary with version `v3.5.0` and docker image pushed to
`cloudscalech/cloudscale-csi-plugin:v3.5.0`
